...
function AnalogContent() {
  ...
  useEffect(() => {
    ...
    fetch(`https://restaurant-brain-production.up.railway.app/client/${encodeURIComponent(slug)}/config`, { cache: "no-store" })
      ...
      .then(d => { if (d?.restaurant_id) setRid(d.restaurant_id) })
      .catch(() => {})
  }, [slug])
  ...
  const confirmAction = useCallback(async (localId: string, action: "seat" | "left") => {
    ...
    if (!row) return
    if (action === "seat") {
      // Show table picker when tableAssignment is on
  ...
  const moveTableGuest = useCallback(async (fromNum: number, toNum: number) => {
    ...
    try {
      const tr = await fetch(`${API}/tables?restaurant_id=${rid}`)
      if (tr.ok) {
    ...
    fetchTables()
  }, [rid, tables, tableOccupants, showToast, fetchTables])
  ...
  const seatToTable = useCallback(async (localId: string, tableNum: number) => {
    ...
    fetchTables()
  }, [rid, rows, tables, patchRow, showToast, fetchTables])
  ...
  }, [rid, tables, seatToTable, showToast])
  ...
  const clearTableOnly = useCallback(async (tableNum: number) => {
    ...
    fetchTables()
  }, [rid, tables, showToast, fetchTables])
    ...
    <div style={{ minHeight: "100dvh", background: V.pageBg, fontFamily: V.font, display: "flex", flexDirection: "column" }}>
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <Link href={`/client/${slug}/station`} style={{ display: "flex", alignItems: "center", color: V.textMuted, textDecoration: "none", padding: "6px 6px", borderRadius: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {/* History */}
          <Link href={`/client/${slug}/history`} style={{ display: "flex", alignItems: "center", gap: 7, padding: "9px 15px", borderRadius: 12, border: `1.5px solid ${V.btnBorder}`, background: visual === "modern" ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.02)", color: V.textSub, textDecoration: "none", fontSize: 13, fontWeight: 700, letterSpacing: "0.01em", touchAction: "manipulation" }}>
