...
async function exportExcel(records: GuestLogRecord[], dateStr: string) {
  ...
  XLSX.writeFile(wb, `HOST_GuestLog_${dateStr}.xlsx`)
}
...
function HistoryContent() {
  ...
  const loadRecords = useCallback(async (dateStr: string) => {
    ...
    if (isToday) {
      try {
        const r = await fetch(`${API}/queue/history?restaurant_id=${rid}`)
        if (r.ok) {
