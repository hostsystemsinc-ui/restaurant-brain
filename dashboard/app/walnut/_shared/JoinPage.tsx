...
interface Props {
  ...
  sessionKey:     string   // unique per location, e.g. "walnut_original"
  totalTables?:   number
}
...
export default function WalnutJoinPage({ restaurantId, restaurantName, sessionKey, totalTables = 16 }: Props) {
  ...
  useEffect(() => {
    const id = sessionStorage.getItem(`host_wait_id_${sessionKey}`)
    if (!id) return
    fetch(`${API}/queue/${id}`)
      ...
      .then(data => {
        if (data && (data.status === "waiting" || data.status === "ready")) {
          ...
          sessionStorage.removeItem(`host_wait_id_${sessionKey}`)
        }
      })
      .catch(() => sessionStorage.removeItem(`host_wait_id_${sessionKey}`))
  }, [router, sessionKey])
  ...
  const submit = async () => {
    ...
    try {
      ...
      sessionStorage.setItem(`host_wait_id_${sessionKey}`, data.entry.id)
      setJoined(true)
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", padding: "16px 24px 0", flexShrink: 0 }}>
        ...
        <img src={LOGO} alt="The Walnut Cafe" style={{ height: 72, objectFit: "contain", display: "inline-block" }} />
      </div>

      {/* Restaurant badge + live info */}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 11, padding: "16px 22px 0", overflow: "hidden" }}>
        ...
        <div style={{ flexShrink: 0 }}>
          ...
          <input className="wj-input" type="tel" placeholder="(720) 000-0000" value={phone}
            ...
            autoComplete="tel"
            style={{ width: "100%", borderRadius: 13, padding: "13px 15px", color: DARK, fontSize: ".95rem", boxSizing: "border-box", caretColor: DARK }} />
          <p style={{ fontSize: 10, color: DARK3, marginTop: 6 }}>By providing your number you agree to receive SMS updates. Reply STOP to opt out.</p>
        </div>
      </div>
      ...
      <div style={{ padding: "14px 22px 28px", flexShrink: 0 }}>
        ...
        <p style={{ textAlign: "center", marginTop: 10, fontSize: ".66rem", color: DARK3, letterSpacing: ".06em" }}>HOST · No app download needed</p>
      </div>
      ...
      {/* Menu Drawer */}
      {menuOpen && <WalnutMenuDrawer onClose={() => setMenuOpen(false)} restaurantName={restaurantName} />}
    </div>
  )
}
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 50, height: "90dvh", background: "#0D0D0D", borderRadius: "22px 22px 0 0", border: "1px solid rgba(255,255,255,0.09)", borderBottom: "none", display: "flex", flexDirection: "column", overflow: "hidden", animation: "sheetUp 0.42s cubic-bezier(0.32,0.72,0,1) both" }}>
        ...
        {/* Category tabs */}
        <div style={{ display: "flex", gap: 6, padding: "12px 20px", borderBottom: "1px solid rgba(255,255,255,0.07)", flexShrink: 0, overflowX: "auto", WebkitOverflowScrolling: "touch" as never }}>
          {WALNUT_MENU.map((cat, i) => (
            <button key={cat.label} onClick={() => setActiveTab(i)}
              style={{ padding: "7px 16px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: ".78rem", fontWeight: 700, whiteSpace: "nowrap" as never, flexShrink: 0, background: activeTab === i ? "rgba(255,255,255,0.92)" : "rgba(255,255,255,0.08)", color: activeTab === i ? "#0D0D0D" : "rgba(255,255,255,0.55)", transition: "background .15s, color .15s" }}>
              {cat.label}
            </button>
          ))}
        </div>
              ...
              {section.items.map((item, ii) => (
                <div key={item.name}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, paddingBottom: 14 }}>
                    ...
                    <p style={{ fontSize: 14, fontWeight: 600, color: "rgba(255,255,255,0.6)", whiteSpace: "nowrap" as never, marginTop: 2, flexShrink: 0 }}>{item.price}</p>
                  </div>
