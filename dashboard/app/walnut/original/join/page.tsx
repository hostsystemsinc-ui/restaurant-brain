...
function OriginalJoinInner() {
  ...
  useEffect(() => {
    const id = sessionStorage.getItem("host_wait_id_walnut_original")
    if (!id) return
    fetch(`${API}/queue/${id}`)
      ...
      .then(data => {
        ...
        } else { sessionStorage.removeItem("host_wait_id_walnut_original") }
      })
      .catch(() => sessionStorage.removeItem("host_wait_id_walnut_original"))
  }, [router])
  ...
  const submit = async () => {
    ...
    try {
      ...
      sessionStorage.setItem("host_wait_id_walnut_original", data.entry.id)
      setJoined(true)
